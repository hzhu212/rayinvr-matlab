...
- tick 2 以上: 
    + zm 异常，第5层应为 3 0 0...，但实际为 3 3 0...；
    + xvel 1 第4层异常，应为 10 0 0...，但实际为 0 10 0...；
    + 即每个应当用 1 个数字表示的层，都用了左端和右端 2 个数字表示，实际只用右端 1 个数字即可！
...
fun_auto 错误 / 正确 -> fun_autotr 错误 / 正确 -> fun_trace 错误 / 正确 返回 npt=7
    fun_dstep 正确
    fun_check 正确
    fun_rkdumb 正确
    fun_vel 正确
...
- 将所有函数中的fun_goto...子函数中的 run(rayinvr_par) 等语句替换为直接声明的 global 语句；
- 按照耗时摘要优化最耗时的函数；
- 去网上查找MATLAB程序优化提速方法；