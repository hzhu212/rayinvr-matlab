...
function main(pathIn, pathOut)
	...
	% 如果imodf不等于1，表明速度模型保存在r.in文件的最后部分；否则，有专门的v.in文件保存
	if imodf ~= 1
		% ?... iunit=10 read(10,1)
	...
	% 读取 f.in 文件到 xfrefl,zfrefl,ivarf 三个二维数组中（类似v.in的结构）
	...
	% 设置搜索模式的最大搜索次数
	% nsmax: 数组，记录每个 ray group 在搜索模式下的最大搜索次数，与数组 ray 一一对应
	if nsmax(1) < 0
		% 350 cycle
		nsmax(1:ngroup) = 10;
	else
	...
	if ~isGoto1000 % -------------------- ~ go to 1000 block start
		...
		for is = 1:nshot % 60
			...
			% 不是第一个炮点
				...
				% 如果当前炮点与上一个炮点是同一个(记录中，同一个炮点但 direction 不同会被记录2次)
