...
C 810        continue 
C          else     
C            if(abs(xshotr-xsec).lt..001.and.abs(zshotr-zsec).lt..001)
C      +       then 
C              if(iflags.eq.1) go to 60
C              ics=0
C              if(id.ne.idsec) then
C                do 830 i=1,nlayer
C                   tang(i,1)=999. 
C                   tang(i,2)=999. 
C                   tang(i,3)=999.
...
C          if(ics.eq.1) then
C c                 
C            call xzpt(xshotr,zshotr,layer1,iblk1,iflags)
C c                 
C            if(iflags.eq.1) then
C              write(11,95)
C 95       format('***  location of shot point outside model  ***')
C              go to 60 
C            end if 
C c                 
C            if((imod.eq.1.or.iray.gt.0.or.irays.eq.1).and.isep.gt.1) then 
...
C c                 
C          do 70 i=1,ngroup
C             if(iraysl.eq.1) then
C               irpos=(ishotr(is)-1)*ngroup+i
C               if(irayt(irpos).eq.0) then
C                 irbnd=irbnd+nrbnd(i)
C                 ictbnd=ictbnd+ncbnd(i)
C                 nrayr=0
C                 go to 70
C               end if
C             end if
...
C             fid=float(id)
C             fid1=fid
C             do 250 j=1,prefl+1
C                refll(j)=0 
C 250         continue
C             do 251 j=1,pconv+1
C                icbnd(j)=-1
C 251         continue
C             ifam=ifam+1
C             nrayr=nray(i)
C             iflagl=0
C             ibrka(ifam)=ibreak(i)
C             ivraya(ifam)=ivray(i)
C             do 870 j=1,nlayer
C                iheadf(j)=ihead(j)
C 870         continue
C             idl=int(ray(i))
C             idt=int((ray(i)-float(idl))*10.+.5)
C             if(idt.eq.2) then
C               if(nrbnd(i).gt.(prefl-1)) then
C                 write(6,125)
...
C               end if
C               refll(1)=idl
C               if(nrbnd(i).gt.0) then
C                 do 230 j=1,nrbnd(i)
C                    irbnd=irbnd+1
C                    refll(j+1)=rbnd(irbnd)
C 230             continue
C               end if
C             else
C               if(nrbnd(i).gt.0) then
C                 do 240 j=1,nrbnd(i)
C                    irbnd=irbnd+1
C                    refll(j)=rbnd(irbnd)
C 240             continue
C               end if
C             end if
C             if(idt.eq.3) then
...
C               ihdwm=-1
C             end if
C             if(ncbnd(i).gt.0) then
C               do 630 j=1,ncbnd(i)
C                  ictbnd=ictbnd+1
C                  icbnd(j)=cbnd(ictbnd)
C 630           continue
C             end if
C             idifff=0
C c
C             if(ircol.eq.1) irrcol=colour(mod(ivray(i)-1,ncol)+1)
C             if(ircol.eq.2) irrcol=colour(mod(is-1,ncol)+1)
C             if(ircol.eq.3) irrcol=colour(mod(i-1,ncol)+1)
C             if(ircol.lt.0) irrcol=-ircol
C c               
C             if(nrayr.le.0) go to 69
C c
C             if(i2pt.gt.0) then
C               iflag2=0
C               nsfc=1
...
C               uf=upf(isf)
C               irayf=ipf(isf)
C               if(irayf.lt.0) go to 1200
C               if(irayf.eq.0) then
C                 xshotf=xf
C                 idf=sign(1.,tf)
C                 if(abs(xshotr-xshotf).lt..001.and.idr(is).eq.idf) then
C                   i2flag=1
C                   isf=isf+1
C                 else
C                   i2flag=0
...
C                 end if
C               else
C                 if(i2flag.eq.1.and.ivray(i).eq.irayf) then
C                   iflag2=1
C                   go to 1200
C                 end if
C                 isf=isf+1
C               end if
C               go to 1110
C 1200          if(iflag2.eq.0) then
C                 nrayr=0
C                 go to 69
C               end if
C             end if
C c
C             call auto(xshotr,zshotr,i,ifam,idl,idt,aminr,amaxr,
C      +         aamin,aamax,layer1,iblk1,aainc,aaimin,nsmax(i),
C      +         iflag,iturn(i),amin(ia0),amax(ia0),ia0,stol,
C      +         irays,nskip,idot,irayps,xsmax,istep,nsmin(i))
C c               
C             if(iflag.ne.0) then
C               write(11,65) ishotw(is),ray(i)
C 65            format('***  shot#',i4,' ray code',f5.1,
C      +               ' no rays traced  ***')
C               nrayr=0
C               nrayl=nrayl+1
C               iflagl=1
C               go to 69 
C             end if 
C             if(amaxr.eq.aminr.and.ihdwf.ne.1) then
C               if(nrayr.gt.1) then
C                 write(11,665) ishotw(is),ray(i)
C 665             format('***  shot#',i4,' ray code',f5.1,
C      +                 ' 1 ray traced  ***')
C                 nrayl=nrayl+1
C                 iflagl=1
C               end if
...
C             if(nrayr.gt.1) then
C               if(idt.eq.2) then
C                 if(amaxr.le.aminr) amaxr=90.
C               end if
C               if(space(i).gt.0.) then
C                 pinc=space(i)
C               else
C                 if(idt.eq.2) then
C                   pinc=2.
...
C                 end if
C               end if
C               ainc=(amaxr-aminr)/float(nrayr-1)**pinc
C             else
C               pinc=1.
C               ainc=0.
C             end if
...
C             nc2pt=0
C c
C             if(i2pt.gt.0.and.nrayr.gt.1) then
C               ii2pt=i2pt
C               ni2pt=1
C               no2pt=0
C               nco2pt=0
...
C                 if(i2flag.eq.1.and.ivray(i).eq.irayf) then
C                   no2pt=no2pt+1
C                   if(no2pt.gt.min(pnrayf,pnobsf)) then
C                     write(6,896) 
C 896                 format('***  pnrayf or pnobsf exceeded  ***')
C                     go to 1199
C                   end if
C                   xo2pt(no2pt)=xf
C                   ifo2pt(no2pt)=0
...
C             i1ray=1
C 90          ir=ir+1
C             if(ir.gt.nrayr.and.ni2pt.le.1) go to 890
C                if(i2pt.eq.0.and.iend.eq.1) go to 890
C                ircbnd=1
C                iccbnd=1
C                iwave=1
C                ihdw=0
C                if(icbnd(1).eq.0) then
C                  iwave=-iwave
C                  iccbnd=2
C                end if
C                nptbnd=0
...
C                fid1=fid
C                iturnt=0
C                if(nc2pt.le.1) then
C                  angled=aminr+ainc*float(ir-1)**pinc
C                  if(amaxr.gt.aminr) then
C                    if(angled.gt.amaxr) then
C                      angled=amaxr
C                      iend=1
C                    end if
C                  else
C                    if(angled.lt.amaxr) then
C                      angled=amaxr
C                      iend=1
C                    end if
C                  end if
C                  if(ir.eq.nrayr.and.nrayr.gt.1) angled=amaxr
C                else
C 891              nco2pt=nco2pt+1
C                  if(nco2pt.gt.no2pt) go to 890
C                  if(ifo2pt(nco2pt).ne.0.and.ii2pt.gt.0) go to 891
C                  xobs=xo2pt(nco2pt) 
C                  tt2min=1.e10
C                  do 892 j=1,nc2pt-1
C                     if((ra2pt(j).ge.xobs.and.ra2pt(j+1).le.xobs).or.
C      +              (ra2pt(j).le.xobs.and.ra2pt(j+1).ge.xobs)) then
C                       denom=ra2pt(j+1)-ra2pt(j)
C                       if(denom.ne.0.) then
C                         tpos=(tt2pt(j+1)-tt2pt(j))/denom*
C      +                       (xobs-ra2pt(j))+tt2pt(j)  
C                       else
C                         tpos=(tt2pt(j+1)+tt2pt(j))/2.
C                       end if
C                       if(tpos.lt.tt2min) then
C                         tt2min=tpos
C                         if(denom.ne.0.) then
C                           aort=(ta2pt(j+1)-ta2pt(j))/denom*
C      +                           (xobs-ra2pt(j))+ta2pt(j)  
C                         else
C                           aort=(ta2pt(j+1)+ta2pt(j))/2.
C                         end if
C                         if(ihdwf.ne.1) then
C                           angled=aort
C                         else
C                           tdhw=aort
C                           hws=tdhw
C                         end if
C                         xdiff=min(abs(xobs-ra2pt(j)),
C      +                            abs(xobs-ra2pt(j+1)))
C                         if(xdiff.lt.x2pt) ifo2pt(nco2pt)=1
C                       end if
C                     end if
C 892              continue
C                  if(tt2min.gt.1.e9.or.(ifo2pt(nco2pt).ne.0.and.
C      +           ii2pt.gt.0)) go to 891
C                end if
C                angle=fid*(90.-angled)/pi18
C                if(ir.gt.1.and.ihdwf.ne.1) then
C                  if(angle.eq.am) go to 90
C                end if
C                am=angle
C                if((fid1*angle).lt.0.) then
C                  id=-id
C                  fid=float(id) 
C                end if
C                layer=layer1
...
C                vp(1,1)=0.0
C                vs(1,1)=0.0
C                vp(1,2)=vel(xshotr,zshotr)
C                vs(1,2)=vp(1,2)*vsvp(layer1,iblk1)
...
C                idray(2)=1
C c
C                if(ii2pt.gt.0) then
C                  irs=0
C                else
C                  irs=ir
C                end if
C                if(invr.eq.1.and.irs.gt.0) then
C                  ninv=ninv+1
C                  do 80 j=1,nvar
C                     fpart(ninv,j)=0.
C 80               continue
...
C                nhskip=0
C c                 
C                call trace(npt,ifam,irs,iturnt,invr,xsmax,iflag,idl,idt,
C      +                    iray,ii2pt,i1ray,modout)
C c                 
C                call ttime(ishotw(is),xshotr,npt,irs,angled,ifam,itt,
C      +                    iszero,iflag,uf,irayf)
C c                 
C c              if(irs.eq.0.and.vr(npt,2).gt.0.) then
C                if(irs.eq.0) then
C                  ic2pt=ic2pt+1
C                  if(ihdwf.ne.1) then
C                    ta2pt(ic2pt)=angled
C                  else
C                    ta2pt(ic2pt)=tdhw-hws
C                  end if
C                  ra2pt(ic2pt)=xr(npt)
C                  tt2pt(ic2pt)=timer
C                  if(vr(npt,2).le.0.) tt2pt(ic2pt)=1.e20
C                end if
C c
C                if(((iray.eq.1.or.(iray.eq.2.and.vr(npt,2).gt.0.)).
C      +         and.mod(ir-1,nrskip).eq.0.and.irs.gt.0).or.
C      +         (irays.eq.1.and.irs.eq.0)) then
C                  call pltray(npt,max(nskip,nhskip),idot,irayps,istep,
C      +                       angled)
C                  if(i33.eq.1) then
C                    if(iszero.eq.1) then
C                      xwr=abs(xshtar(ntt-1)-xobs)
C                    else
C                      xwr=xobs
C                    end if
C                    write(33,335) xshtar(ntt-1),ivraya(ifam),i,xwr,
C      +                           tt(ntt-1)
C 335                format(f12.4,2i4,3f12.4)
C                  end if
C                end if
C c
C                if(vr(npt,2).gt.0.and.irs.gt.0.and.abs(modout).ge.2) 
C      +             call cells(npt,xmmin,dxmod,dzmod)
C c                 
C                if(invr.eq.1.and.irs.gt.0) call fxtinv(npt)
C c
C                if(ihdwf.eq.0) go to 890
C                if(ntt.gt.pray) then
C                  write(6,995)
C 995              format(/
C      +     '***  max number of rays reaching surface exceeded  ***'/)
C                  go to 890 
C                end if 
C             go to 90
C c                 
...
C             if(ii2pt.gt.0) then
C               nc2pt=ic2pt
C               if(ni2pt.gt.1) then
C                 call sort3(ta2pt,ipos,nc2pt)
C                 do 893 j=1,nc2pt
C                    ho2pt(j)=ra2pt(j)
C 893             continue
C                 do 894 j=1,nc2pt
C                    ra2pt(j)=ho2pt(ipos(j)) 
C 894             continue
C                 do 897 j=1,nc2pt
C                    ho2pt(j)=tt2pt(j)
C 897             continue
C                 do 898 j=1,nc2pt
C                    tt2pt(j)=ho2pt(ipos(j)) 
C 898             continue
C               end if
C               ni2pt=ni2pt+1
C               if(ni2pt.gt.n2pt) ii2pt=0
C               nco2pt=0
C               go to 91
C             end if
C c
C             if(ninv.gt.0) call calprt(xshotr,i,ivraya(ifam),
C      +                    idr(is),ximax,iflagw,iszero,x2pt)
C             if(iflagw.eq.1) iflagi=1
C c
C             if(iray.gt.0.or.irays.eq.1) call empty
C c
C             nrayr=nrg
C 69          if(iflagl.eq.1) then
C               flag='*'
C             else
C               flag=' '
C             end if
C             write(6,375) ishotw(is),ray(i),nrayr,flag
C 375         format('shot#',i4,':   ray code',f5.1,':   ',
C      +             i3,' rays traced ',a1)
C c
C             if(ntt.gt.pray) go to 1000
C c                 
C 70       continue 
C c                 
...
C 60    continue    
C c                 
C 1000  if((isep.lt.2.or.isep.eq.3).and.((itx.gt.0.and.ntt.gt.1).or.
C      +  idata.ne.0.or.itxout.gt.0)) then
...
C       end if
C c
C       if(itxout.gt.0) write(17,930) 0.,0.,0.,-1
C c
C       if(irkc.eq.1) then
C         write(6,75)
C         write(11,75)
C 75      format(/'***  possible inaccuracies in rngkta  ***')
C       end if
C c                 
C       if(nrayl.gt.0) then
C         write(6,785) nrayl
C         write(11,785) nrayl
C 785     format(/
C      +'***  less than nray rays traced for ',i4,' ray groups  ***')
C       end if
C c                 
C       if(iflagi.eq.1) write(6,775)
C 775   format(/'***  attempt to interpolate over ximax  ***')
C c
C       if(i33.eq.1) then
C         do 1033 i=1,narinv
C            write(34,335) xscalc(i),abs(icalc(i)),ircalc(i),xcalc(i),
C      +                   tcalc(i)
C 1033    continue
C       end if
C c
C       if(abs(modout).ne.0) call 
C      + modwr(modout,dxmod,dzmod,modi,ifrbnd,frz,xmmin,xmmax)
C c
C       if(ifd.gt.0) call fd(dxzmod,xmmin,xmmax,ifd)
C c                 
900   ntblk=0     
      do 920 i=1,nlayer
         ntblk=ntblk+nblk(i)
